...
void printArray(Particle **grid_array, size_t *sizes, size_t pnc){

    for (int i = 0; i < pnc; i++){
        printf("  ---PRINTING CELL ARRAY No. %d:---\n", i);
        if(sizes[i] == 0){
        ...
        for(int j = 0; j < sizes[i]; j++){
            ...
            printf("  Particle of body %c at %.1f | %.1f\n", grid_array[i][j].body, grid_array[i][j].x.s[0], 
                                                                        grid_array[i][j].x.s[1]);
            if(j == sizes[i] - 1){
...
Particle **prepGrid(ParticleList **grid, size_t N, size_t pnc, size_t *sizes, cl_context *context)
{
    // Allocate memory for an array containing an array
    // Overallocation needed, each array needs size N
    ...
    for (int i = 0; i < pnc; i++)
    {
        ...
        // loop through list and copy the Particle p into the array
...
int main()
{
    ...
    // prep
    size_t sizes[pnc];
    Particle **argarray = prepGrid(grid, N, pnc, sizes, &context);
    //prepArray(grid, pnc, nc, N, sizes, &context);


    printArray(argarray, sizes, pnc);
    // prep end
    
    for(int i = 0; i < pnc; i++){
        CL_CHECK(clSetKernelArgSVMPointer(kernel[0], 0, argarray[i]));
        printf("did arg\n");
